...
function HandoffLive({ session }: { session: HandoffSession }) {
  ...
  // Clock + scripted transcript ticker
  useEffect(() => {
    if (ended) return;
    const iv = window.setInterval(() => {
      ...
      // Reveal scripted turns whose timestamp has passed
      ...
      // Sentiment random-walk anchored to script beats
      setSentiment((prev) => {
        ...
        const next = [...prev.slice(-59), target];
        return next;
      });
    }, TICK_MS);
  ...
  // Auto-check compliance items at scripted timestamps
  useEffect(() => {
    setCompliance((prev) => {
      let changed = false;
      const next = { ...prev };
