...
export const Route = createFileRoute("/prompt-studio")({
  validateSearch: (search: Record<string, unknown>): { unansweredId?: string; note?: string } => ({
        ...
        {loading ? (
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[1fr_320px]">
            <div className="min-h-0 overflow-y-auto p-5">
              ...
              {tab === "voice" && (
                <VoicePanel value={voice} onChange={setVoice} voices={voices.length ? voices : undefined} />
              )}
              {tab === "guardrails" && (
