...
export function LatencyChart({ points }: { points: DailyPoint[] }) {
  ...
  const series = [
    { id: "p50", label: "p50", values: points.map((p) => Math.round(p.latencyP50)), color: "#82b536" },
    { id: "p90", label: "p90", values: points.map((p) => Math.round(p.latencyP90)), color: "#f68909" },
    { id: "p99", label: "p99", values: points.map((p) => Math.round(p.latencyP99)), color: "#e2483d" },
  ];
