...
export function Simulator({ rules }: { rules: Rule[] }) {
  ...
  const update = (key: keyof SimContext, val: unknown) =>
    setCtx({ ...ctx, [key]: val } as SimContext);
              ...
              <div key={f.key}>
                ...
                {f.type === "enum" ? (
                  ...
                  <div className="mt-075">
                    <Switch
                      aria-label={f.label}
                      checked={Boolean(ctx[f.key as keyof SimContext])}
                      onCheckedChange={(v) => update(f.key as keyof SimContext, v)}
                    />
                  </div>
                ) : (
                  <Input
                        ...
                        <span
                          ...
                          className={cn(
                            ...
                            c.matched
                              ...
                              : "bg-background-danger-subtler text-text-danger",
                          )}
                        >
                          {c.label}
                        </span>
