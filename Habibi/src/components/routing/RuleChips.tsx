  ...
  return (
    <span
      ...
      className={cn(
        ...
        matched === true && "border-border-success bg-background-success-subtler text-text-success-bolder",
        matched === false && "border-border-danger-subtle bg-background-danger-subtler text-text-danger-bolder",
      )}
    >
...
export function RuleChips({ rule }: { rule: Rule }) {
  return (
    <div className="flex flex-wrap items-center gap-075 text-body-small">
      <span className="rounded bg-background-brand-subtlest px-075 py-025 font-semibold text-text-brand">IF</span>
            ...
            <span className="flex flex-wrap items-center gap-075 rounded-medium border border-dashed border-border px-075 py-025">
              {node.or.map((c, i) => (
                <span key={c.id} className="flex items-center gap-075">
                  {i > 0 && <span className="text-body-small font-semibold text-text-warning">OR</span>}
                  {condChip(c)}
                </span>
              ))}
            </span>
      ...
      <span className="ml-050 rounded bg-background-brand-bold px-075 py-025 font-semibold text-white">THEN</span>
      <span className="rounded-medium border border-border bg-surface px-100 py-025 font-medium text-text">
        ...
        {rule.then.params && Object.entries(rule.then.params).map(([k, v]) => (
          <span key={k} className="ml-050 text-text-subtlest">· {k}={v}</span>
        ))}
      </span>
    </div>
  );
