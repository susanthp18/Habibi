...
export function RuleEditor({ rule, onSave, onCancel, onSaveAndTest }: Props) {
  ...
  const removeNode = (idx: number) =>
    setDraft({ ...draft, when: draft.when.filter((_, i) => i !== idx) });
  const addAnd = () => setDraft({ ...draft, when: [...draft.when, blankCondition()] });
  const addOr = () =>
    setDraft({
      ...draft,
      when: [...draft.when, { id: cid(), or: [blankCondition(), blankCondition()] }],
    });
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1 space-y-200 overflow-y-auto p-200">
        ...
        <div>
          ...
          <Textarea
            className="mt-050 min-h-[3.75rem] text-body-small"
        ...
        <div>
          ...
          <div className="mt-100 space-y-100 rounded-large border border-border bg-surface-sunken/50 p-100">
            {draft.when.length === 0 && (
              <div className="p-150 text-center text-body-small text-text-subtlest">
            ...
            {draft.when.map((node, idx) => (
              <div key={node.id} className="rounded-medium border border-border bg-surface p-100">
                {idx > 0 && (
                  <div className="mb-050 text-body-small font-semibold text-text-brand">AND</div>
                )}
                {"or" in node ? (
                  <div className="space-y-075">
                    <div className="text-body-small font-semibold text-text-warning">
                      Match ANY of:
                    </div>
                    {node.or.map((c, ci) => (
                      <ConditionRow
                    ...
                    <Button
                      ...
                      onClick={() =>
                        updateNode(idx, { ...node, or: [...node.or, blankCondition()] })
                      }
                    >
                      <Plus className="mr-050 h-3 w-3" />
                      OR condition
                    </Button>
                  </div>
                ) : (
            ...
            <Input
              ...
              onChange={(e) =>
                setDraft({
                  ...
                  then: {
                    ...draft.then,
                    params: { ...(draft.then.params ?? {}), team: e.target.value },
                  },
                })
              }
            />
            ...
            <Input
              ...
              onChange={(e) =>
                setDraft({
                  ...
                  then: {
                    ...draft.then,
                    params: { ...(draft.then.params ?? {}), template: e.target.value },
                  },
                })
              }
            />
            ...
            <Input
              ...
              onChange={(e) =>
                setDraft({
                  ...
                  then: {
                    ...draft.then,
                    params: { ...(draft.then.params ?? {}), doc: e.target.value },
                  },
                })
              }
            />
      ...
      <div className="flex shrink-0 items-center justify-end gap-100 border-t border-border bg-surface px-200 py-150">
        ...
        <Button variant="outline" size="sm" onClick={() => onSaveAndTest(draft)}>
          Save & Test
        </Button>
        <Button
          size="sm"
          className="bg-background-brand-bold hover:bg-background-brand-bold-pressed"
