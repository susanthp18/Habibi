...
export function AuditLog({ entries }: { entries: AuditEntry[] }) {
  ...
  const rows = entries
    ...
    .filter(
      (e) =>
        !q ||
        e.ruleName.toLowerCase().includes(q.toLowerCase()) ||
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-150">
        <ul className="space-y-100">
          {rows.map((e) => (
            <li key={e.id} className="rounded-large border border-border bg-surface p-150">
              <div className="flex items-center gap-100">
                ...
                <span className="flex-1 truncate text-body-small font-medium text-text">
                  {e.ruleName}
                </span>
          ...
          {rows.length === 0 && (
            <li className="p-200 text-center text-body-small text-text-subtlest">No entries.</li>
          )}
        </ul>
      </div>
    </div>
