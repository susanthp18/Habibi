...
import { fmtDelta, fmtRate, useOfferHealth, useTunerSuggestions, type OfferHealthWindow, type TunerSuggestions } from "@/api/offer-health";
    ...
    <div className="shrink-0 rounded-large border border-border bg-surface">
      ...
      >
        <div className="flex items-center gap-075 text-body-small font-semibold text-text">
          {open ? <ChevronDown className="h-3.5 w-3.5 text-text-subtlest" /> : <ChevronRight className="h-3.5 w-3.5 text-text-subtlest" />}
        ...
        <div className="flex items-center gap-075">
          ...
          {alerts.length > 0 ? <Lozenge tone="warning">{alerts.length} alert{alerts.length === 1 ? "" : "s"}</Lozenge> : null}
        </div>
      </button>
      {!open ? null : (
      <>
      <div className="grid grid-cols-2 gap-150 border-t border-border px-200 py-150 md:grid-cols-4 xl:grid-cols-6">
        <Metric label="Coverage" value={<Rate value={data.funnel.coverage} />} hint={fmtDelta(data.funnel.coverageChange)} />
        <Metric label="Presented" value={<Rate value={data.funnel.presentationRate} />} hint={`${data.volume.presented} spoken`} />
        <Metric label="Interest" value={<Rate value={data.funnel.interestRate} />} hint={`${data.volume.customers} customers`} />
        <Metric label="Close probe" value={<Rate value={data.closeProbe.conversion} />} hint={`${data.closeProbe.captured} captured`} />
        <Metric
          label="AHT delta"
          value={
            <span className={cn("tabular", (data.guardrails.ahtDeltaSec ?? 0) > 25 ? "text-text-warning" : "text-text")}>
              {data.guardrails.ahtDeltaSec == null ? "—" : `${data.guardrails.ahtDeltaSec >= 0 ? "+" : ""}${Math.round(data.guardrails.ahtDeltaSec)}s`}
            </span>
          }
        ...
        <Metric
          label="Top suppress"
          value={<span className="truncate text-body-small">{topSuppress?.reason?.replaceAll("_", " ") ?? "—"}</span>}
      ...
      {dark ? (
        <p className="border-t border-border px-200 py-100 text-body-small text-text-subtle">
          Nothing in <span className="font-mono">offer_decisions</span> for this window. The engine only
          runs inside a live bot conversation, so an idle bot produces an empty panel — this is not a
      ...
      {alerts.length > 0 ? (
        <ul className="space-y-050 border-t border-border px-200 py-100">
          {alerts.map((a) => (
            <li key={a.metric} className="flex items-start gap-075 text-body-small text-text-warning">
...
function Metric({ label, value, hint }: { label: string; value: ReactNode; hint?: string }) {
  return (
    <div className="min-w-0">
      ...
      <div className="mt-025 text-[1.05rem] font-semibold leading-tight text-text">{value}</div>
  ...
  return (
    <div className="border-t border-border px-200 py-100">
      <div className="flex flex-wrap items-center gap-075">
        ...
        <span className="text-caption text-text-subtle">{tuner?.note ?? "loading"}</span>
      </div>
      {items.length === 0 ? (
        <p className="mt-050 text-caption text-text-subtlest">
          No weight changes suggested. Copy to env is a human step — this never writes live knobs.
        </p>
      ) : (
        <ul className="mt-075 space-y-025 font-mono text-caption text-text">
          {items.map((k) => (
            <li key={k.name}>
