...
const TOGGLES: Array<{ key: keyof Guardrails; label: string; hint: string }> = [
  ...
  },
  {
    ...
    hint: "Flags missing disclosure on turn 1",
  },
  {
        ...
        <div>
          ...
          <div className="divide-y divide-border rounded-medium border border-border bg-surface">
            {TOGGLES.map((t) => (
              <div
                key={t.key as string}
                className="flex items-start justify-between gap-150 px-150 py-150"
              >
                <div>
                  <div className="text-body-small font-medium text-text">{t.label}</div>
      ...
      <div className="flex flex-col gap-250">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-075">
            ...
            {!rulesLoading && rulesFailed && (
              <div className="col-span-2 rounded-medium border border-dashed border-border px-150 py-150 text-body-small text-text-subtlest">
                Could not load redaction rules — this list is not a statement about what is
                redacted.
              </div>
            )}
          ...
          <p className="mt-050 text-body-small text-text-subtlest">
            Read-only here — toggle PAN, account, Aadhaar, etc. in Redaction Hub. Studio cannot
            override them.
          </p>
        </div>
      </div>
