  ...
  const columns = useMemo<RecordsColumn<ActiveCall>[]>(
    () => [
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-075">
              ...
              {r.offerPolicy && r.offerPolicy.status !== "none" ? (
                <Lozenge tone={OFFER_STATUS_TONE[r.offerPolicy.status]}>{OFFER_STATUS_LABEL[r.offerPolicy.status]}</Lozenge>
              ) : null}
              ...
              {r.liveQa && r.liveQa.status && r.liveQa.status !== "none" ? (
                <Lozenge tone={LIVE_QA_STATUS_TONE[r.liveQa.status] ?? "warning"}>
                  {LIVE_QA_STATUS_LABEL[r.liveQa.status] ?? r.liveQa.reason?.replace(/-/g, " ") ?? "Live QA"}
                </Lozenge>
              ) : null}
            </div>
      ...
      },
      {
        ...
        align: "right",
        cell: (r) => <span className="tabular text-body-small font-semibold text-text">{fmtDur(r.durationSec)}</span>,
      },
      {
        id: "sentiment",
        header: "Sentiment",
      ...
      },
      {
        ...
        header: "Last line",
        cell: (r) => (
          <p className="line-clamp-1 max-w-[22rem] text-body-small italic text-text-subtle">“{r.lastLine}”</p>
        ),
      },
    ],
    [],
