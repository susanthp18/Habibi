...
export function CalibrationView({ sessions, onClose }: { sessions: CalibrationSession[]; onClose: (id: string) => void }) {
  ...
  if (!active) {
    return <div className="rounded-large border border-border bg-surface p-300 text-center text-body-small text-text-subtlest">No calibration sessions.</div>;
  }
  ...
  return (
    <div className="grid gap-150 lg:grid-cols-[240px_minmax(0,1fr)]">
      <div className="space-y-050 rounded-large border border-border bg-surface p-100">
        <div className="px-100 pb-050 text-body-small font-semibold text-text-subtlest">Sessions</div>
        {sessions.map((s) => (
          <button
            key={s.id}
            ...
            className={cn(
              ...
              s.id === active.id ? "bg-background-brand-subtlest text-text-brand" : "hover:bg-surface-sunken text-text",
            )}
          >
            <div className="font-medium">{s.name}</div>
            <div className="text-body-small text-text-subtlest">{s.customerName} · {s.reviewers.length} reviewers · {s.status}</div>
          </button>
        ))}
      </div>
      ...
      <div className="rounded-large border border-border bg-surface">
        <div className="flex items-center justify-between border-b border-border px-150 py-100">
          <div>
            ...
            <div className="text-body-small text-text-subtlest">Target score for {active.customerName}: <ScoreBand total={targetTotal} size="sm" /></div>
          </div>
            ...
            <thead className="bg-surface-sunken text-body-small text-text-subtlest">
              <tr>
                ...
                {defaultRubric.sections.map((s) => (
                  <th key={s.id} className="px-150 py-100 text-left font-medium">{s.label}</th>
                ))}
            ...
            <tbody className="divide-y divide-border">
              <tr className="bg-background-brand-subtlest/40">
                ...
                {defaultRubric.sections.map((s) => {
                  ...
                  return <td key={s.id} className="px-150 py-100 text-text">{sub.toFixed(0)}</td>;
                })}
                <td className="px-150 py-100 text-right"><ScoreBand total={targetTotal} size="sm" /></td>
              ...
              {active.reviewers.map((r) => {
                ...
                return (
                  <tr key={r.reviewer}>
                    ...
                    <td className="px-150 py-100 text-right"><ScoreBand total={total} size="sm" /></td>
                    <td className={cn(
