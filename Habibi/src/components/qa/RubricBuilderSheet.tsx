...
export function RubricBuilderSheet({
  ...
  const addCriterion = (sectionId: string) => {
    ...
    updateSection(sectionId, {
      criteria: [...(draft.sections.find((s) => s.id === sectionId)?.criteria ?? []), c],
    });
  };
  const updateCriterion = (sectionId: string, cid: string, patch: Partial<RubricCriterion>) => {
    ...
    if (!section) return;
    updateSection(sectionId, {
      criteria: section.criteria.map((c) => (c.id === cid ? { ...c, ...patch } : c)),
    });
  };
  ...
  const addSection = () => {
    setDraft({
      ...
      sections: [
        ...draft.sections,
        { id: `s-${Date.now()}`, label: "New section", weight: 0, criteria: [] },
      ],
    });
  };
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      ...
      >
        ...
        <div className="border-b border-border bg-surface px-200 py-100">
          <div className="mb-050 flex items-center justify-between text-body-small text-text-subtle">
            ...
            <span
              className={cn(
                "font-semibold",
                validSum ? "text-text-success-bolder" : "text-text-danger-bolder",
              )}
            >
              {totalWeight} / 100
            </span>
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-surface-sunken">
            <div
              className={cn(
                "h-full",
                validSum ? "bg-background-success-bold" : "bg-background-danger-bold",
              )}
        ...
        <div className="flex-1 space-y-150 overflow-y-auto p-200">
            ...
            <div key={section.id} className="rounded-large border border-border">
              <div className="flex items-center gap-100 border-b border-border bg-surface-sunken px-150 py-100">
                ...
                <button
                  ...
                  className="rounded p-050 text-text-danger hover:bg-background-danger-subtler"
                >
              ...
              <div className="divide-y divide-border">
                {section.criteria.map((c) => (
                  <div
                    key={c.id}
                    className="grid grid-cols-[1fr_60px_28px] gap-100 px-150 py-100 text-body-small"
                  >
                    <div>
                      <input
                        value={c.label}
                        onChange={(e) =>
                          updateCriterion(section.id, c.id, { label: e.target.value })
                        }
                      ...
                      <input
                        value={c.description}
                        onChange={(e) =>
                          updateCriterion(section.id, c.id, { description: e.target.value })
                        }
                      ...
                      <label className="mt-050 inline-flex items-center gap-050 text-body-small text-text-subtle">
                        <input
                          ...
                          checked={!!c.critical}
                          onChange={(e) =>
                            updateCriterion(section.id, c.id, { critical: e.target.checked })
                          }
                        />
                    ...
                    <input
                      ...
                      value={c.weight}
                      onChange={(e) =>
                        updateCriterion(section.id, c.id, { weight: Number(e.target.value) })
                      }
                      className="h-fit rounded border border-border bg-surface px-050 py-050 text-right"
                    />
                    <button
                      onClick={() => removeCriterion(section.id, c.id)}
                      className="h-fit rounded p-050 text-text-danger hover:bg-background-danger-subtler"
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="flex w-full items-center gap-050 px-150 py-100 text-body-small text-text-brand hover:bg-background-brand-subtlest"
                >
          ...
          <button
            onClick={addSection}
            className="flex w-full items-center justify-center gap-050 rounded-large border border-dashed border-border py-100 text-body-small text-text-brand hover:bg-background-brand-subtlest"
          >
