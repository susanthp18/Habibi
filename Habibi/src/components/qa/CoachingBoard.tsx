...
const COLS: Array<{ key: CoachingStatus; label: string; tint: string }> = [
  { key: "assigned", label: "Assigned", tint: "bg-background-warning-subtler text-text-warning-bolder" },
  { key: "in_progress", label: "In progress", tint: "bg-background-brand-subtlest text-text-brand" },
  ...
  return (
    <div className="space-y-150">
      <div className="flex items-center justify-between">
        <div className="text-body-small text-text-subtle">Assign actions to agents; drag between columns to update status.</div>
        <button
          onClick={onNew}
            ...
            >
              <div className="flex items-center justify-between border-b border-border px-150 py-100">
                <div className="flex items-center gap-100">
                  <span className={cn("rounded-full px-100 py-025 text-body-small font-semibold", col.tint)}>{col.label}</span>
              ...
              <div className="flex-1 space-y-100 p-100">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-100">
                        <div className="text-[0.75rem] font-medium text-text">{a.title}</div>
                        <ChevronRight className="h-3.5 w-3.5 shrink-0 text-text-subtlest opacity-0 group-hover:opacity-100" />
                      </div>
                      <div className="mt-050 text-body-small text-text-subtle">{a.agentId} · {a.category}</div>
                      <div className="mt-075 flex items-center gap-100 text-body-small">
                        <span className={cn("inline-flex items-center gap-050", overdue ? "text-text-danger-bolder" : "text-text-subtlest")}>
                ...
                {items.length === 0 && (
                  <div className="p-300 text-center text-body-small text-text-subtlest">Drop cards here.</div>
                )}
              </div>
            </div>
          );
