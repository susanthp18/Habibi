...
import { getAuthUserApiV1UserAuthUserGet } from "@/agentstudio/client/sdk.gen";
import { getWorkflowCountApiV1WorkflowCountGet } from "@/agentstudio/client/sdk.gen";
import { impersonateApiV1SuperuserImpersonatePost } from "@/agentstudio/client/sdk.gen";
...
export async function impersonateAsSuperadmin(params: {
  ...
  // via NEXT_PUBLIC_APP_URL (e.g. #); falls back to the current
  // origin (e.g. localhost, staging, or already on the app).
  const appBaseUrl = import.meta.env.VITE_AGENTSTUDIO_APP_URL || window.location.origin;
