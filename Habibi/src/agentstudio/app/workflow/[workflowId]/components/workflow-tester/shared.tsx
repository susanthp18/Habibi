...
export function DisabledNotice({ reason }: { reason: string }) {
    return (
        <div className="rounded-lg border border-border-warning/80 bg-background-warning/80 px-3 py-2.5 text-sm text-text-warning dark:border-border-warning/20 dark:bg-background-warning-bold/10 dark:text-text-warning">
            <div className="flex items-start gap-3">
                <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
                <div className="space-y-0.5">
                    <p className="font-medium">Testing is paused</p>
                    <p className="text-text-warning/90 dark:text-text-warning">{reason}</p>
                </div>
            </div>
        </div>
    );
