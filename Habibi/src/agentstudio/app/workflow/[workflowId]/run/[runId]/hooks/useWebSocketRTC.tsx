...
import { getTurnCredentialsApiV1TurnCredentialsGet, validateUserConfigurationsApiV1UserConfigurationsUserValidateGet, validateWorkflowApiV1WorkflowWorkflowIdValidatePost } from "@/agentstudio/client/sdk.gen";
import { TurnCredentialsResponse } from "@/agentstudio/client/types.gen";
import { WorkflowValidationError } from "@/agentstudio/components/flow/types";
import type { ConversationNodeTransitionItem, RealtimeFeedbackMessage as FeedbackMessage } from "@/agentstudio/components/workflow/conversation";
...
import { signalingUrl } from '@/agentstudio-host/signaling-url';
...
export const useWebSocketRTC = ({ workflowId, workflowRunId, accessToken, initialContextVariables, onNodeTransition }: UseWebSocketRTCProps) => {
    ...
    const getWebSocketUrl = useCallback(() => {
        ...
        // is no longer needed.
        return signalingUrl(workflowId, workflowRunId, accessToken);
            ...
            if (response.error) {
                ...
                const message = detailFromError(
                    ...
                    isServiceUnavailable
                        ? 'PayInt Voice Studio is temporarily unavailable. Please try again later.'
                        : 'API Key Error',
                );

                if (isServiceUnavailable) {
                    // MPS is a PayInt Voice Studio-owned dependency. Do not tell the customer
                    // to change credentials when PayInt Voice Studio could not validate them.
