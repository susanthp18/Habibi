...
import Link from '@/agentstudio-host/shims/next-link';
import { useParams } from '@/agentstudio-host/shims/next-navigation';
import posthog from '@/agentstudio-host/shims/posthog';
import { useEffect, useRef, useState } from 'react';
import { toast } from 'sonner';

import WorkflowLayout from '@/agentstudio/app/workflow/WorkflowLayout';
import RunProvenance from '@/agentstudio-host/RunProvenance';
import {
    getWorkflowApiV1WorkflowFetchWorkflowIdGet,
    getWorkflowRunApiV1WorkflowWorkflowIdRunsRunIdGet,
} from '@/agentstudio/client/sdk.gen';
import { MediaPreviewButton, MediaPreviewDialog } from '@/agentstudio/components/MediaPreviewDialog';
import { OnboardingTooltip } from '@/agentstudio/components/onboarding/OnboardingTooltip';
import { Button } from '@/agentstudio/components/ui/button';
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from '@/agentstudio/components/ui/card';
import { Skeleton } from '@/agentstudio/components/ui/skeleton';
import { ConversationRailFrame, RealtimeFeedback, WorkflowRunLogs } from '@/agentstudio/components/workflow/conversation';
import { PostHogEvent } from '@/agentstudio/constants/posthog-events';
import { WORKFLOW_RUN_MODES } from '@/agentstudio/constants/workflowRunModes';
...
import { downloadFile, getSignedUrl } from '@/agentstudio/lib/files';
                ...
                peaks.map((peak, index) => (
                    <span
                        ...
                        className={cn(
                            ...
                            track === 'user' ? 'bg-background-information-bold' : 'bg-background-success-bold'
                        )}
                    ...
                    <div className="flex items-center gap-2" role="group" aria-label="Playback tracks">
                        <Button
                            ...
                            className={cn(
                                'gap-1.5',
                                userTrackActive
                                    ? 'border-border-information bg-background-information text-text-information hover:bg-background-information dark:border-border-information/60 dark:bg-background-information-bold/30 dark:text-text-information'
                        ...
                        <Button
                            ...
                            className={cn(
                                'gap-1.5',
                                botTrackActive
                                    ? 'border-border-success bg-background-success text-text-success hover:bg-background-success dark:border-border-success/60 dark:bg-background-success-bold/30 dark:text-text-success'
                                    : 'text-muted-foreground opacity-60'
                            )}
                        >
                        ...
                        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div className="min-w-0 flex-1 space-y-2">
                                ...
                                <div className="flex min-w-0 items-center gap-4 pt-1">
                                    ...
                                    <div className={`h-8 w-8 rounded-full flex items-center justify-center ${isTextChatRun ? 'bg-background-information-bold/15' : 'bg-background-success-bold/20'}`}>
                                        {isTextChatRun ? (
                                            <FileText className="h-5 w-5 text-text-information" />
                                        ) : (
                                            <svg className="h-5 w-5 text-text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24">
