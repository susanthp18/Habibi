...
import { testToolApiV1ToolsToolUuidTestPost } from "@/agentstudio/client/sdk.gen";
import type { ToolTestResponse } from "@/agentstudio/client/types.gen";
import type { HttpMethod, PresetToolParameter, ToolParameter } from "@/agentstudio/components/http";
                ...
                <DialogContent className="max-h-[90vh] max-w-3xl grid-rows-[auto_minmax(0,1fr)]">
                    ...
                    <div className="space-y-6 overflow-y-auto pr-1">
                        ...
                        {isUnsafeHttpMethod(httpMethod) && (
                            <div
                                role="alert"
                                className="flex gap-3 rounded-lg border border-border-warning bg-background-warning p-4 text-text-warning dark:border-border-warning dark:bg-background-warning-bold/40 dark:text-text-warning"
                            >
                        ...
                        <div className="space-y-3 border-t pt-4">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Resolved values that PayInt Voice Studio would normally derive from each configured preset.
                                </p>
                            </div>
                            <ParameterFields
                        ...
                        {result && (
                            <div className="space-y-3 border-t pt-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <span
                                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${
                                            isSuccess
                                                ? "bg-background-success text-text-success dark:bg-background-success-bold/30 dark:text-text-success"
                                                : "bg-background-danger text-text-danger dark:bg-background-danger-bold/30 dark:text-text-danger"
                                        }`}
                                    >
                                        <span>{isSuccess ? "✓" : "✗"}</span>
                                        {resultBadgeLabel}
                                ...
                                {result.hint && (
                                    <div className="rounded border border-border-warning bg-background-warning p-3 text-sm text-text-warning dark:border-border-warning/40 dark:bg-background-warning-bold/20 dark:text-text-warning">
                                        {result.hint}
                                    </div>
                                )}
                                {result.error && (
