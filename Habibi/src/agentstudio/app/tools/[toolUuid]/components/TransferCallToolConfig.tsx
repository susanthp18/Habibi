...
import type { RecordingResponseSchema } from "@/agentstudio/client/types.gen";
import { RecordingSelect, StaticTextWarning } from "@/agentstudio/components/flow/TextOrAudioInput";
import {
    CredentialSelector,
    KeyValueEditor,
    type KeyValueItem,
    ...
    type ToolParameter,
    UrlInput,
} from "@/agentstudio/components/http";
...
import { DOCS_BASE } from "@/agentstudio/constants/documentation";
                    ...
                    >
                        ...
                        <TabsContent value="dynamic" className="space-y-5 mt-4">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    PayInt Voice Studio sends the resolved argument dictionary to this endpoint. The endpoint must return transfer_context.destination and may return transfer_context.custom_message.{" "}
                                    <a
                            ...
                            <div className="grid gap-2">
                                ...
                                />
                                <Label className="text-xs text-muted-foreground">
                                    PayInt Voice Studio sends a POST request with the resolved argument dictionary.
                                </Label>
                            </div>
                            ...
                            <div className="grid gap-2">
                                ...
                                />
                                <Label className="text-xs text-muted-foreground">
                                    Spoken while PayInt Voice Studio waits for the resolver response.
                                </Label>
                            </div>
                            ...
                            <div className="grid gap-2 pt-4 border-t">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Add values PayInt Voice Studio injects at runtime. These are not exposed to the LLM and can use templates like {`{{initial_context.state}}`} or {`{{gathered_context.state}}`}.{" "}
                                    <a
                                        href={`${DOCS_BASE}/voice-agent/tools/call-transfer#dynamic-resolver-request`}
                                        target="_blank"
                                        rel="noopener noreferrer"
