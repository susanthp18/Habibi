...
import {
    ...
    type ServiceSegment,
} from "@/agentstudio/components/ServiceConfigurationForm";
...
import { VoiceSelectorModal } from "@/agentstudio/components/VoiceSelectorModal";
import { LANGUAGE_DISPLAY_NAMES } from "@/agentstudio/constants/languages";
import { formatRoundingPolicy } from "@/agentstudio/lib/billingDisplay";
...
function preferredMode(
    ...
    if (configuration?.mode === "dograh") return WHITELABEL ? "byok" : "dograh";
    if (configuration?.mode === "byok") {
        return asRecord(configuration.byok)?.mode === "realtime" ? "realtime" : "byok";
    }
    if (isDograhEffectiveConfig(effectiveConfiguration)) return WHITELABEL ? "byok" : "dograh";
...
function ThirdPartyProviderNotice() {
    return (
        <div className="mt-4 flex gap-3 rounded-md border border-border-warning/40 bg-background-warning-bold/10 px-4 py-3 text-sm text-text-warning dark:text-text-warning">
            <Info className="mt-0.5 h-4 w-4 shrink-0" />
            <div>
                <p className="font-medium">Third-party provider data notice</p>
                <p className="mt-1 leading-6">
                    PayInt Voice Studio sends data required by the selected model service. This may include prompts,
    ...
    return (
        <Card className="mb-4 border-primary/20 bg-primary/[0.03]">
            <CardContent className="space-y-2 pt-5 text-sm">
                ...
                {dograhModelPrice && (
                    <MetricPrice label="PayInt Voice Studio model usage" price={dograhModelPrice} />
                )}
                {thirdPartyModels && (
                    <p className="text-muted-foreground">
...
export function AIModelConfigurationV2Editor({
    ...
    const [mode, setMode] = useState<ModelMode>(WHITELABEL ? "byok" : "dograh");
    ...
    const saveDograhConfiguration = async () => {
        ...
        try {
            if (
                ...
                throw new Error(
                    `PayInt Voice Studio speed must be between ${dograhSpeedRange.min} and ${dograhSpeedRange.max}.`,
                );
            }
            await onSave({
                version: 2,
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs value={mode} onValueChange={(value) => setMode(value as ModelMode)} className="space-y-6">
                <TabsList className={`grid w-full ${WHITELABEL ? "grid-cols-2" : "grid-cols-3"}`}>
                    <TabsTrigger value="realtime">Speech to Speech</TabsTrigger>
                    {!WHITELABEL && <TabsTrigger value="dograh">PayInt Voice Studio</TabsTrigger>}
                ...
                {!WHITELABEL && <TabsContent value="dograh" className="mt-0">
                    <p className="mb-4 text-sm text-muted-foreground">
                        PayInt Voice Studio provides a managed transcriber, LLM, and voice pipeline. Select a voice and language while PayInt Voice Studio manages the underlying model providers.{" "}
