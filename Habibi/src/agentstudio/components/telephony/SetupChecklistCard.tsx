  ...
  return (
    <Card>
      <CardHeader className="space-y-1">
        ...
        <CardDescription>
          {connectivity === "sip"
            ? "PayInt Voice Studio provides the SIP connection; you connect your own carrier and numbers to it."
            : "Finish these steps to place and receive calls on this configuration."}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {!checklist.ready_for_outbound && (
          <div className="flex items-start gap-3 rounded-md border border-border-warning bg-background-warning p-4 text-text-warning dark:border-border-warning dark:bg-background-warning-bold dark:text-text-warning">
              ...
              <li key={step.key} className="flex items-start gap-3">
                <span
                  ...
                  className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${
                    step.complete
                      ? "border-border-success bg-background-success-bold text-white"
                      : blocking
