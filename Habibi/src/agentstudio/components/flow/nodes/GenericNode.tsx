...
import { useWorkflow } from "@/agentstudio/app/workflow/[workflowId]/contexts/WorkflowContext";
import type { NodeSpec } from "@/agentstudio/client/types.gen";
import { DocumentBadges } from "@/agentstudio/components/flow/DocumentBadges";
import { NodeEditForm, useNodeSpecs } from "@/agentstudio/components/flow/renderer";
import { ToolBadges } from "@/agentstudio/components/flow/ToolBadges";
import { FlowNodeData } from "@/agentstudio/components/flow/types";
...
import { NODE_DOCUMENTATION_URLS } from "@/agentstudio/constants/documentation";
import { useAppConfig } from "@/agentstudio-host/app-config";
import { copyTextToClipboard } from "@/agentstudio/lib/clipboard";
import { cn } from "@/agentstudio/lib/utils";
import { createUuid } from "@/agentstudio/lib/uuid";
import { resolveWebhookBaseUrl } from "@/agentstudio/lib/webhookUrl";
...
function getBadgeForSpec(
    ...
    if (!spec) {
        return { label: "Node", className: "bg-background-neutral-bold text-white" };
    }

    switch (variant) {
        case "start":
            return { label: "Start Node", className: "bg-background-success-bold text-white" };
        case "agent":
            return { label: "Agent Node", className: "bg-background-information-bold text-white" };
        case "end":
            return { label: "End Node", className: "bg-background-danger-bold text-white" };
        case "global":
            return { label: "Global Node", className: "bg-background-warning-bold text-white" };
        case "trigger":
            return { label: "API Trigger", className: "bg-background-discovery-bold text-white" };
        case "webhook":
            return { label: "Webhook", className: "bg-background-information-bold text-white" };
        case "qa":
            return { label: "QA Analysis", className: "bg-background-success-bold text-white" };
        case "integration":
            return { label: spec.display_name, className: "bg-background-information-bold text-white" };
    }
}
    ...
    return (
        <div className="flex items-center gap-1.5">
            <Circle
                className={`h-2 w-2 ${
                    enabled
                        ? "fill-icon-success text-text-success"
                        : "fill-gray-400 text-text-subtle"
                }`}
            />
