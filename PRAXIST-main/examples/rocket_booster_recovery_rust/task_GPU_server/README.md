# Rocket Booster Recovery (Rust) Praxist task

This directory is the Rust edition of the high-concurrency Rocket Booster
Recovery server configuration and can be launched as a Praxist task harness.
The directory name is retained for task compatibility. The evaluator,
candidate-controller API, 6DoF simulation, metric computation, and resource
observer run on CPU in Rust; they do not use Python, NumPy, JAX, XLA, CUDA, a
GPU, or a virtual environment.
...
The Rust complete baseline reproduces `495 / 12,288 = 4.0283203125%`.
Complete evidence and the compact ledger are under `assets/baselines/`; they
are baseline assets, not historical run records. Runtime-created
...
`vendor/` directory and `Cargo.lock` support `--locked --offline` builds
without a host-private Cargo cache. The Praxist orchestrator runs in its own
environment; this task has neither a simulator venv nor a task venv. A shared
server may select a managed Rust toolchain through the ordinary shell
environment before Praxist starts, but machine-specific absolute paths do not
belong in the task harness.
...
cargo run --release --locked --offline \
  ...
  --mode complete \
  --output-dir task_GPU_server/scratch/manual-baseline-complete \
  --threads 16
```
...
supported command template from the prompt. The candidate static audit is not
an operating-system sandbox; do not run untrusted candidate code on a host
with sensitive credentials or mounts.
