...
use rocket_booster_recovery_task::candidate_api::{
    ControlOutput, State, Vec2, Vec3, clip, controller_quat_to_rot, cross, dot, mat_t_vec,
    safe_norm, safe_norm2, scale, unit,
};
...
pub fn control_step(
    ...
    cfg: &ControllerConfig,
) -> ControlOutput<ControllerMemory> {
...
pub fn validate_config(cfg: &ControllerConfig) -> Result<(), String> {
    cfg.validate().map_err(|error| error.to_string())
}

pub fn previous_actuators(memory: &ControllerMemory) -> ([f32; 2], [f32; 2], f32) {
    (
        memory.prev_gimbal_rad,
        memory.prev_grid_rad,
        memory.prev_throttle,
