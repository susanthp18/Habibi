...
| **Customer & Account** | `customers`, `customer_notes`, `products`, `product_eligibility_rules`, `accounts`, `ledger_entries`, `emi_installments`, `payment_events` |
| **Consent** | `consent_records`, `channel_consents`, `optout_events`, `contact_events`, `contact_day_counters` |
...
| **Collections workflow** | `work_items` *(view)*, `promises`, `promise_reminders`, `promise_installments`, `payment_plans`, `payment_intents`, `followups`, `disputes`, `dispute_evidence`, `document_requests`, `document_templates`, `document_files`, `document_delivery_attempts`, `callbacks`, `callback_reminders`, `treatment_holds`, `treatment_decisions`, `authority_decisions` |
...
- `payment_events` — first-class NACH/UPI/ECS bounce (the row *is* the case). Idempotent on `(tenant_id, source, source_ref)`. Open/in_progress rows project into `work_items` as `entity_type='bounce'`. Cured when a ledger payment covers the installment. `used_this_week` / contact caps still come from `contact_events` (P6); a bounce pay-link is `statutory` / `bounce_notice`.

**Consent**
- `consent_records` — 1:1 with customer: DND registry flag, expiry, allowed contact window (days + hours).
- `channel_consents` — per-channel status (opted_in/opted_out/dnd/expired) + weekly frequency cap. `consent_id`. `used_this_week` is a cache of the contact ledger, not a hand-edited counter.
...
- `contact_events` — append-only outbound ledger (bot + human + system). `purpose` (`outreach` / `statutory` / `in_session`), `outcome` (`allowed` / `denied`), `touch_counted`. Source of truth for daily/weekly caps.
- `contact_day_counters` — `(customer_id, local_date)` atomic budget; locked inside `contact_policy.admit()`.
...
- `conversations` — the text-transport layer (WhatsApp/SMS/email) for the Inbox. Carries a **NOT NULL `interaction_id` set at creation** — the underlying `interactions` row is created the moment a chat session starts (`status='active'`), so live chats are on the spine immediately, not only once archived. `customer_id`, `assigned_user_id`, `status` (`bot` / `needs_human` / `escalated` / `assigned`). **"Mine" is not stored** — the UI derives it as `assigned_user_id === currentUser` from `GET /me`.
- `messages` — thread messages. `conversation_id`, `sender` (`customer` / `bot` / `agent` / `system`), `body`, `delivery_status`, provider refs.
...
- `supervisor_actions` - listen-in, whisper, barge, and force-handoff audit. `interaction_id`, `supervisor_user_id`, action, target refs, note, `consumed_at` (whisper drain), `audio_joined` (Twilio takeover succeeded), created timestamp.
- `live_qa_decisions` — append-only log of the live FPC engine (P7). One row per failing turn / barge recommendation, including shadow. `verdict` ∈ `pass | fail_soft | fail_critical`; `recommended_action` ∈ `none | listen | whisper | barge | inbox`. Floor, Handoff and the examiner pack read the latest snapshot. Auto-barge in `LIVE_QA_BARGE_MODE=live` marks `enacted`.

**Collections workflow**
- `work_items` — **a read-only VIEW, not a base table.** Projects the open queue across disputes, callbacks, document requests, broken PTPs, leads, followups, and open bounces into one shape (`entity_type`, `entity_id`, `assignee_user_id`, `status`, `priority`, `sla_due_at`, `source`) by `UNION`-ing the domain tables. The domain tables stay the single source of truth; the view holds no independent state, so there is nothing to sync and no status can drift. (If per-queue-item state ever appears that has no home on the parent — e.g. a workspace-only snooze — promote it to a thin `work_item_overrides` table keyed by `(entity_type, entity_id)` rather than duplicating status.)
...
- `treatment_holds` — the collections veto, as a row. `kind` ∈ `hardship | dispute | complaint | bereavement | legal`; `customer_id`, optional `account_id` (NULL = the whole borrower), `source`, `specialist_user_id`, `sla_due_at`, `expires_at`, `released_at`. One live hold per `(customer, account, kind)` via a partial unique index over `COALESCE(account_id,'')` — Postgres treats NULLs as distinct, so without the COALESCE two customer-level hardship holds would both be allowed. Read by the treatment engine (`agent_core/treatment/policy.py`) **and** by the offer engine, which is where the Digital Lending Guidelines' collection-vs-upsell separation is enforced. `legal` still permits a statutory notice; `dispute` still permits a specialist call about the dispute itself.
- `treatment_decisions` — append-only log of every next-best-treatment invocation, modelled on `offer_decisions`. Written for suppressed and shadow decisions too: a log holding only the contacts we made has no negative class in it and cannot answer "why did the engine go quiet on Tuesday?". Carries the ranked `candidates` (each with the feature `vector` as it was at decision time), the per-action `excluded` reasons, `chosen_action`/`chosen_channel`/`scheduled_at`, `expected_value` in rupees, `rationale`, and `enacted`/`outcome`. `GET /treatment/insights` is the report over it; `GET /treatment/cases` groups it by case — `(customer, trigger_kind, trigger_ref)` — into the ladder a floor lead reads. `created_at` is stamped with `clock_timestamp()`, not `now()`: this is an append-only log, and two decisions written in one transaction would otherwise share a timestamp with no way left to tell which came second. `outcome` is written by `agent_core/treatment/followthrough.py` — including for *shadow* decisions, where it can only ever be `paid`/`ptp`/`superseded`, never `no_answer`, because nothing was sent.
