...
> **Numbering settled 2026-09-09.** This table is the design's proposal and is *not* what shipped. Two ids were built with different meanings before the table was written, and they are now authoritative: **`G-F4` is `handoff_is_an_edge`** (a granted transfer has somewhere to go) and **`G-F7` is `carry_is_fact_only`**; `G-F11` `text_walkability` matches. The design's "namespace + leaveability" and "gate monotonicity" are renumbered **`G-F15`** and **`G-F16`** and are reserved but unbuilt. `agent_core/cards/compile.py` now carries a `_GATE_NAMES` registry that `_gate()` asserts against, so an id cannot acquire a second meaning again — it caught a live case on its first run, `G-OB1` emitting as both `missions_declared` and `outbound`. Gate monotonicity was largely pre-empted by adding `handoff_to_agent` to the shared identity floor, which closes the same hole for every card rather than per edge.
