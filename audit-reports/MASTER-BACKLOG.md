...
| **Affected files** | `backend/outbound.py:743-751` · `backend/campaigns.py:611-626` · **plus an Alembic migration — see below** |
| **CORRECTED 2026-09-04** | **This is not a ~15-line change.** The `parked` state the strategy calls for cannot be written: `ck_campaign_targets_state CHECK (state = ANY (ARRAY['pending','dialing','done','failed','skipped']))`. Adding it needs a **migration on a regulated table**, which the entry never mentioned — and per `WP-001`, 52 of 102 migrations write rows and 9 downgrades are no-ops, so it inherits that rollback caveat. **Re-scope before dispatching.** Either widen the CHECK in a migration, or reuse an existing state plus a separate `reconciliation_required` flag and avoid the schema change entirely. That choice is a decision, not an implementation detail. |
